'''
2025.9.25更新：使用两种思路插值
方1：加权,结果见back5
方2：直接插值,求scale,结果见back4

本代码是方2，先求】插值，再加权平均，结果存储在文件夹
code/part_2/outcome_for_part2/SDCH_20240203_0071_middle.back2
code/part_2/outcome_for_part2/SDCK_20240203_0071_middle.back2
'''
...
def process_data(wavelength,spectrum,var,save_path,wave_record_path=None):
    var=var**0.5 
    uncer=var  #标准差，石山了
    ...
    with open(wave_record_path,'r') as f:
        lines=f.readlines() # 读取所有行
        record_wave = [line.strip().split(',') for line in lines]  # 去除换行符并按逗号分割
    ...
    for i in range(len(wavelength)-1):
        ...
        number_1, number_2 = record_wave[i][0]  , record_wave[i][1]
        number_1, number_2 = float(number_1), float(number_2)
        print(f"用户选择的两个点：{number_1}, {number_2}")
        ####################################################################################
        ####交互结束，应该单独成一个函数######################################################
        # # 解析两次点击的横坐标
        ...
        #uncer_001=var[i][head_index_1:middle_index_1+1]
        ...
        #
        uncer_001 = var[i][head_index_1:middle_index_1+1]
        weights = (1 / uncer_001)**2  # 使用误差的倒数平方作为权重
        weighted_average_1 = np.sum(spectrum[i][head_index_1:middle_index_1+1] * weights) / np.sum(weights)
        print(f"weighted_average_1={weighted_average_1}, average_1={average_1}")
        average_1=weighted_average_1
        

        temp_spectrum=[]
        temp_uncer=[]
        for j in range(head_index_1,middle_index_1):
        #for j in range(middle_index_2,tail_index_2):
            #先插值################这里修改两种方法切换##############
            spectrum_new=interp_1d(wavelength[i+1],spectrum[i+1],wavelength[i][j],kind='linear',extrapolate=True)
            # print(spectrum_new,wavelength[i][j])
            # print(wavelength[i+1])
            # print(spectrum[i+1],count_nan(spectrum[i+1])[0],count_nan(spectrum[i+1])[1])  #
            # aaaaa
            print(wavelength[i][j],spectrum_new)
            uncer_new=interp_1d(wavelength[i+1],var[i+1],wavelength[i][j],kind='linear',extrapolate=True)
            temp_spectrum.append(spectrum_new)
            temp_uncer.append(uncer_new)

        print("插值后nan个数：",count_nan(temp_spectrum)[0],count_nan(temp_spectrum)[1])
        ...
        #print(temp_spectrum)
            ...
            # temp_spectrum.append((wavelength[i+1][j],spectrum[i+1][j]))
            # temp_uncer.append((wavelength[i+1][j],var[i+1][j]))
        #average_2=np.mean(spectrum[i+1][middle_index_2:tail_index_2+1])  #各自加权平均
        ...
        # 计算加权平均
        #
        #uncer_002 = var[i+1][middle_index_2:tail_index_2+1]
        #weights = (1 / uncer_002)**2  # 使用误差的倒数平方作为权重
        #weighted_average_2 = np.sum(spectrum[i+1][middle_index_2:tail_index_2+1] * weights) / np.sum(weights)
        #print(f"weighted_average_2={weighted_average_2}, average_2={average_2}")
        #average_2=weighted_average_2
        ...
        # 计算权重
        weights = (1 / np.array(temp_uncer) )**2  # 使用误差的倒数平方作为权重

        # # 计算加权平均
        weighted_average_spectrum = np.sum(temp_spectrum * weights) / np.sum(weights)

        print("加权平均值2:", weighted_average_spectrum,"普通平均值2:", average_2)
        average_2=weighted_average_spectrum
...
if __name__=='__main__':
    ...
    record_path='code/part_2/outcome_for_part2/record_wave_H.txt'
    record_path='code/part_2/outcome_for_part2/record_wave_K.txt'
    ...
    wavelength_id, spectrum_id, var_id=process_data(wavelength,spectrum,var,'code/part_2/outcome_for_part2/SDCK_20240203_0071_middle.back2',record_path)
    wavelength_id, spectrum_id, var_id=sort(wavelength_id, spectrum_id, var_id)

    make_csv('code/part_2/outcome_for_part2/SDCK_20240203_0071_middle.back2/outcome.csv', wavelength_id, spectrum_id, var_id)