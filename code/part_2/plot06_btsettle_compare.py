...
if __name__ == '__main__':
    ...
    plt.plot(wavelength[find_insert_position(wavelength,1.50):find_insert_position(wavelength,1.75)],flux[find_insert_position(wavelength,1.50):find_insert_position(wavelength,1.75)],label='Original BT-Settl',color='blue')
    plt.plot(wavelength_settle_H,flux_settle_H,label='BT-Settl H-modified',color='orange')
    plt.xlabel('Wavelength (μm)')
    plt.ylabel('Flux')
    plt.title('BT-Settl Model Comparison - H Band')
    plt.legend()
    plt.grid()
    plt.savefig('code/part_2/outcome_for_part2/middle_figure/btsettle_H_compare.png')
    plt.show()


    #K波段绘制对比图
    plt.figure(figsize=(10,6))
    plt.plot(wavelength[find_insert_position(wavelength,2.0):find_insert_position(wavelength,2.4)],flux[find_insert_position(wavelength,2.0):find_insert_position(wavelength,2.4)],label='Original BT-Settl',color='blue')
    plt.plot(wavelength_settle_K,flux_settle_K,label='BT-Settl K-modified',color='green')
    plt.xlabel('Wavelength (μm)')
    plt.ylabel('Flux')
    plt.title('BT-Settl Model Comparison - K Band')
    plt.legend()
    plt.grid()
    plt.savefig('code/part_2/outcome_for_part2/middle_figure/btsettle_K_compare.png')
    plt.show()